...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'email_list' AND policyname = 'Public can insert email list'
  ) THEN
    CREATE POLICY "Public can insert email list"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'contact_submissions' AND policyname = 'Public can insert contact submissions'
  ) THEN
